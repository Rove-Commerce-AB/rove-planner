...
import { getCustomerRates, createCustomerRate } from "@/lib/customerRates";
...
export function AddCustomerRateModal({
  ...
  useEffect(() => {
    ...
    setLoading(true);
    setError(null);
    Promise.all([getRoles(), getCustomerRates(customerId)])
      .then(([r, rates]) => {
        setRoles(r);
  ...
  const handleSubmit = async () => {
    ...
    setSubmitting(true);
    try {
      await createCustomerRate(customerId, selectedRoleId, rateNum);
      setSelectedRoleId("");
      setRate("");
      onSuccess();
      onClose();
