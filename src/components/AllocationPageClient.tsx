                    ...
                    >
                      ...
                      {row.percentByWeek.map((pct, i) => {
                        ...
                        return (
                          <td
                            key={`${w.year}-${w.week}`}
                            className={`${showLeftBorder ? "border-l border-grid-light-subtle " : ""}${hasBooking ? "border-r border-grid-light-subtle" : ""} px-1 py-1 text-center select-none cursor-crosshair ${row.consultant.unavailableByWeek[i] ? "!bg-[var(--color-border-default)] text-text-primary" : getAllocationCellBgClass(pct)} ${isCurrentWeek(w) && !row.consultant.unavailableByWeek[i] ? "current-week-cell border-l border-r bg-brand-signal/15" : ""} ${isCurrentWeek(w) && row.consultant.unavailableByWeek[i] ? "current-week-cell border-l border-r" : ""} hover:!bg-brand-blue/50 ${isDragRange ? "!bg-brand-lilac/50" : ""} ${isDragRange ? "border-brand-signal border-t border-b" : ""} ${isDragLeft ? "border-l-2" : ""} ${isDragRight ? "border-r-2" : ""}`}
                            title={title}
                            onMouseDown={(e) => {
                              e.preventDefault();
